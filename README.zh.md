...
## Living Web Changelog（实时网络变更日志）

共享的、内容寻址的缓存使一种无状态抓取器无法构建的能力成为可能 —— 一份实时网络的变更日志:

- **标题级时序差分** —— 第一个重新爬取已变更页面的智能体只计算一次逐章节增量，之后每个引用旧 `contentHash` 的智能体都能以约 0 token 继承「18 个章节中只有这 3 个变了」。
- **内容寻址去重 + 镜像合并** —— 相同正文以完整 sha256 作为键，别名与镜像被合并为同一条目，从而提升命中率（跨源映射是一份经过审核的硬编码白名单，绝不从流量中学习）。
- **语义差分层** —— 每次重访一份活文档时，只返回发生变化的章节（通常节省 80~90%）。
- **自适应、波动性驱动的 TTL** —— 废弃固定的 24 小时 TTL。波动性仅从内部内容哈希推导，避免对稳定页面进行冷重爬（上限 7 天，并尊重源站的 ETag/Last-Modified 重新验证）。
- **版本锁定、自动失效的笔记** —— 笔记被锁定到其引用的内容版本，当页面在该章节发生变化时被软标注为「可能已过时」（绝不强制隐藏），从而让集体记忆自我修正。
- **群体「别费劲」索引** —— 仅依据缓存自身测得的客观信号（`spaPartial`／字节数／HTTP 状态）记录 SPA 陷阱／付费墙／死胡同，避免浪费爬取。
