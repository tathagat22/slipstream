...
## Living Web Changelog (살아있는 웹 변경 기록)

공유되는 콘텐츠 주소 기반 캐시는 상태 비저장 페처가 만들 수 없는, 살아있는 웹의 변경 기록을 가능하게 합니다:

- **헤딩 단위 시간 차분** — 변경된 페이지를 처음 다시 크롤링한 에이전트가 섹션별 델타를 한 번만 계산하고, 오래된 `contentHash`를 인용하는 이후의 모든 에이전트는 "18개 섹션 중 이 3개만 변경됨"을 ~0 토큰으로 물려받습니다.
- **콘텐츠 주소 기반 중복 제거 + 미러 통합** — 동일한 본문은 전체 sha256으로 키가 지정되고, 별칭과 미러는 하나의 항목으로 통합되어 히트율을 끌어올립니다(교차 출처 매핑은 트래픽에서 학습되지 않는, 검증된 고정 허용 목록입니다).
- **의미 기반 차분 레이어** — 살아있는 문서를 다시 방문할 때마다 변경된 섹션만 반환합니다(일반적으로 80~90% 절약).
- **적응형 변동성 기반 TTL** — 고정 24시간 TTL을 폐기. 변동성을 내부 콘텐츠 해시에서만 도출하여 안정적인 페이지의 콜드 재크롤링을 피합니다(최대 7일, 출처의 ETag/Last-Modified 재검증을 존중).
- **버전 고정 자가 만료 메모** — 메모는 인용된 콘텐츠 버전에 고정되며, 해당 섹션에서 페이지가 변경되면 "오래되었을 수 있음"으로 소프트 라벨링됩니다(절대 강제로 숨기지 않음). 이를 통해 집단 메모리가 스스로 수정됩니다.
- **하이브 "굳이 하지 마" 인덱스** — SPA 함정/페이월/막다른 길을, 캐시가 스스로 측정한 객관적 신호(`spaPartial`/바이트 수/HTTP 상태)에서만 기록하여 낭비되는 크롤링을 피합니다.
